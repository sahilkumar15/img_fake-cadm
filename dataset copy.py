...
#   1. collect_samples() pre-filters samples where img or source_img is missing
#      on disk. This eliminates all the imread WARN messages AND the NaN/crash
#      in augmentor.py, because bad samples never enter the DataLoader at all.
#   2. collect_samples() skips non-image files (ldm.json, .bak etc).
#   3. collect_samples() skips entries not in ldm.json (safety guard).
#   4. __getitem__ still has a None guard as a second safety net.
#
import os
import cv2
import json
...
class DeepfakeDataset(Dataset):
        ...
        for key in sorted(self.class_dict.keys()):
            ...
            for r, _, filename in sorted(os.walk(d, followlinks=True)):
                for name in sorted(filename):
                    ...
                    info_key = path[:-4]   # strip extension → ldm.json key

                    # Guard 1: must be in ldm.json
                    ...
                    # Guard 2: image file must exist on disk
                    if not os.path.isfile(path):
                        skipped_no_img += 1
                        continue

                    # Guard 3: source image must exist on disk
                    # (fake frames point to original frames which may be missing
                    #  if that original video had no face detected during extraction)
                    ...
                    samples.append(
                        (path, {'labels': class_label,
                                'landmark': landmark,
                                'source_path': source_path,
                                'video_name': video_name})
                    )

        if skipped_no_key or skipped_no_img or skipped_no_src:
            print(f"  [dataset] skipped: {skipped_no_key} not-in-ldm, "
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        # Safety net (should not trigger if collect_samples filtered correctly)
        if img is None or source_img is None:
            return None, "imread returned None"
