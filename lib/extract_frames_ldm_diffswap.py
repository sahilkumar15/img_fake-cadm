...
DATA_PATH = "/scratch/sahil/projects/img_deepfake/datasets/diffswap/extracted"
REAL_PATH = os.path.join(DATA_PATH, "Real")
FAKE_PATH = os.path.join(DATA_PATH, "DiffSwap")
SAVE_IMGS_PATH = "./test_images_diffswap"
...
def parse_label(img_path):
    if "/DiffSwap/" in img_path or img_path.startswith(FAKE_PATH):
        return 1
    return 0
...
def get_source_path(save_path, label):
    # For DiffSwap, use self-path fallback for compatibility.
    return save_path
...
def main():
    ...
    real_imgs = list_images(REAL_PATH)
    fake_imgs = list_images(FAKE_PATH)

    print(f"{len(real_imgs)} images in Real")
    print(f"{len(fake_imgs)} images in DiffSwap")

    all_imgs = real_imgs + fake_imgs
    print(f"Total images: {len(all_imgs)}")

    for img_path in tqdm(all_imgs, desc="Processing DiffSwap"):
...
# rm -rf test_images_diffswap
# mkdir -p test_images_diffswap
# python lib/extract_frames_ldm_diffswap.py
# ls test_images_diffswap/ldm.json